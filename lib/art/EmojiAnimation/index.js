 ...
 * @param {number} [options.emojis = []] - Array of emojis ({base64: string; width: number; height: number;}[])
 *
 * @example
...
function animateEmoji(emojiIndex) {
  // Init
  var rand = Math.floor(Math.random() * 100 + 1);
  var flows = ["flowOne", "flowTwo", "flowThree"];
  var timing = (Math.random() * (1 ) + 1.0).toFixed(1);
  // Animate Particle
  var emoji = window.__emojis[emojiIndex].base64;
  if(!window.__emojis[emojiIndex].base64) {
      return 'EMOJI_BASE64_NOT_FOUND';
  }
  $(
    '<div class="particle part-' + rand"><img src="' + emoji +'"/></div>'
  )
    .appendTo(".particle-box")
    .css({
        height: window.__emojis[emojiIndex].height,
        width: window.__emojis[emojiIndex].width,
...
class EmojiAnimation {
	...
	playEmoji(emojiIndex) {
		this.__webView.evaluateJS(
			`window.animateEmoji(${emojiIndex});`,
			(e) => {}
		);
	}
}
