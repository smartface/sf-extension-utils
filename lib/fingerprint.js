        ...
        return this.getSecureData(FIELDS.USERNAME).read()
            ...
            .then(username => {
                ...
                console.info({
                    ...
                    rememberMe,
                    useFingerprint,
                    isNotFirstTime: !this.isFirstTime,
                    doNotAutoAskLogin
                })
                ...
                return Promise.all(postOps)
                    ...
                    .then(loginHandlerResult => {
                        ...
                        if (canUseFingerprint) {
                            ...
                            const updatePromise = !this.isFirstTime && (isUserNameDifferent || isPasswordDifferent) ? updateCredentailsConfirmation : Promise.resolve;
                            return updatePromise()
                                ...
                                .catch(() => console.log("user chose to not update credentials"));
                        }
                        if (rememberMe) {
                            console.log("saving username for remember me");
                            userNameSec.save({ value: userNameUserEntry });
                        }
                        return loginHandlerResult === "undefined" ? Promise.resolve(loginResult) : Promise.resolve(loginHandlerResult);
                    });
            });
    }
