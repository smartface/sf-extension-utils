...
var isIphoneX = (Screen.height === 812 ? true : false); //detect the iphoneX acc to screen height 
...
function loginWithFingerprint() {
    ...
    if (useFingerprintLogin && System.fingerPrintAvailable && storedUserName) {
        ...
        alert({
            title: (isIphoneX  ? "Use FaceID" : global.lang.useFingerprintTitle || "Use Fingerprint"),
            message: (isIphoneX ? "Would you like to use faceID login on your future logins?" : global.lang.useFingerprintMessage || "Would you like to use fingerprint login on your future logins?"),
            buttons: [{
                text: global.lang.yes || "Yes",
                type: AlertView.Android.ButtonType.POSITIVE,
                onClick: function() {
                ...
                onClick: function() {
                    ...
                    Data.setBooleanVariable(dataKeys.firstLogin,
                        firstLogin = true);
                    return callback((isIphoneX ? "User does not want to use faceID" : "User does not want to use fingerprint"));
                },
            }]
        });
    }
        ...
        if (!System.fingerPrintAvailable)
            ...
            return callback((isIphoneX ? "User does not choose to use faceID login"  : "User does not choose to use fingerprint login"));
    }
    ...
    function validateFingerPrint() {
        System.validateFingerPrint({
            android: {
                title: (isIphoneX ? "Login with faceID" : global.lang.scanFingerprintTitle || "Login with fingerprint")
            },
            message: (isIphoneX ? "Scan your faceID" : global.lang.scanFingerprintMessage || "Scan your finger"),
            onSuccess: function() {
                return callback(null, {
                    password: tbPassword.text || storedPassword,
                    success: onSuccess
            ...
            onError: function() {
                return callback((isIphoneX ? "FaceID validation failed"  : "Fingerprint validation failed"));
            }
        });
    }
