                ...
                if (canUseFingerprint) {
                    ...
                    if (!this.isFirstTime && (isUserNameDifferent || isPasswordDifferent)) {
                        return updateCredentailsConfirmation()
                            .then(() => Promise.resolve(loginResult, true))
                            .catch(() => Promise.resolve(loginResult));
                    }
                }
                return Promise.resolve(loginResult);
            })
            .then((loginResult, updateCredentials) => {
                ...
                try {
                    let loginHandlerResult = loginHandler(loginResult) || loginResult;
