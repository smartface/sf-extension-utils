...
class GoogleSafetyNet {
	 ...
	 * @param {object} options - Cofiguration of GoogleSafetyNet helper object (required)
      * @param {string} options.apiKey - API key from Google APIs (required). <a href="https://developer.android.com/training/safetynet/attestation#obtain-api-key">See here</a> 
	 */
	constructor(options = {}) {
	 ...
	 *
	 * 
	 * You should trust the APK information only if the value of ctsProfileMatch is true. So to validate device is trusted, check JWS's ctsProfileMatch & basicIntegrity boolean variables. Google has a table
	 * to show how device status could affect the values of basicIntegrity and ctsProfileMatch. <a href="https://developer.android.com/training/safetynet/attestation#potential-integrity-verdicts">See here</a> 
	 * 
	 * Note: Do not validate these two variables in source code if the code can be accessed by reverse-engineering 
	 * 
	 * </pre>
	 * 
	 * @method
	 ...
	 * 
	 * if (System.OS === "Android") {
	 *    sendAttestationRequest("a2d0sa1@3sqwe123f12sww").then(
	 *              (jws) => {
	 *                   console.log("JWS " + jws);
	 *              })
	 *         .catch(
	 ...
	 * console.log("isPlayServicesAvailable : " + isPlayServicesAvailable);
	 *
	 */
