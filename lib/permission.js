...
function getPermission(permissions, rationaleDisplay, callback) {
    ...
    if (!(permissions instanceof Array)) {
        assert(assert.isString(permissions) || assert.isNumber(permissions), 'permission must be a string or a number');
    ...
    for (i = permissions.length - 1; i > -1; i--) {
        p = permissions[i];
        assert(assert.isString(p) || assert.isNumber(p), 'permission must be a string or a number');
