...
function getPermission(permissions, rationaleDisplay, callback) {
    ...
    if (permissions.length === 0) {
        callback && callback(permissionStatus.GRANTED); //all granted
        return;
    }
    ...
    function rationalDisplayCallback(err) {
        if (err) {
            callback && callback(permissionStatus.DENIED);
        }
        else {
    ...
    function continueRequestPermissions() {
        ...
        permissionRequestMap[requestPermissionCode] = {
            ...
            result: function(e) {
                if(e.result){
                    callback && callback(permissionStatus.GRANTED);
                }else{
                    var flag = true;
                    for(var i=0 ; i<permissions.length;++i){
                        if (!Application.android.checkPermission(permissions[i]) && !Application.android.shouldShowRequestPermissionRationale(permissions[i])){
                            flag = false;
                            break;
                        }
                    }
                    if(flag) callback && callback(permissionStatus.DENIED);
                    else callback && callback(permissionStatus.NEVER_ASK_AGAIN);
                }
            }
        };
        Application.android.requestPermissions(requestPermissionCode, permissions);
    }
