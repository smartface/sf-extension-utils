...
function getPermission(opts = { androidPermission, permissionText = '', iosPermission }) {
    return new Promise((resolve, reject) => {
        if (System.OS === 'iOS') {
                ...
                if (Multimedia.ios.getCameraAuthorizationStatus() !==
                    ...
                    Multimedia.ios.requestCameraAuthorization((status) => {
                        if (status) {
                            ...
                            alertWrapper(opts.permissionText)
                            reject();
                        }
                    });
                }
            ...
            const prevPermissionRationale = Application.android
                .shouldShowRequestPermissionRationale(opts.androidPermission);
            if (Application.android.checkPermission(opts.androidPermission)) {
                ...
                Application.android.onRequestPermissionsResult = (e) => {
                    ...
                    if (e.requestCode === requestPermissionCode && e.result) {
                        ...
                        setTimeout(() => alertWrapper(opts.permissionText), 500);
                        reject(PERMISSION_STATUS.NEVER_ASK_AGAIN);
                    } else {
                        reject(PERMISSION_STATUS.DENIED);
                    }
                };
                Application.android.requestPermissions(requestPermissionCode, opts.androidPermission);
            }
        }
    });
}
