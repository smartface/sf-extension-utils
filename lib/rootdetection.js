/*globals SF, requireClass, __SF_RootDetection, array*/

/**
 * Smartface RootDetection Module
 * @module rootdetection
 * @type {object}
 * @author Dogan Ekici  <doga.ekici@smartface.io>
...
/**
 * RootDetection includes a few established methods to capture whether device is rooted or not. To know if device is rooted, looking for potentially dangerous app packages/paths,
 * system folder accessibility, su binaries & schemas. The dangerous app packages/paths should be kept upto date by commuity.
 * 
 * @public
 * @class
 * @example
 ...
 * console.log(" The device is found untrusted condition");
 * else
 * console.log(" The device seems safe");
 * 
 */
class RootDetection {
	...
	constructor() {
		if (System.OS === "iOS")
			...
			this.rootDetection = requireClass("io.smartface.android.utils.RootDetectionUtil");
	}
	 ...
	 * }else {
	 * 	console.log("It seems you can trust");
	 * }
	 *
	 */
	isRooted() {
	...
	checkSuBinaryExistance() {
		...
		let isThereSuBinaries = this.rootDetection.checkSuBinaryExistance(array(suBinaryPaths, "java.lang.String"));
