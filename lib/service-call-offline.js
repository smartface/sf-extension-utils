...
class OfflineRequestServiceCall extends ServiceCall {
    ...
    static sendAll() {
        return Promise.resolve().then(() => {
            let allPendingRequests = Data.getStringVarible(TABLE_NAMES.PENDING_REQUESTS);
            allPendingRequests = allPendingRequests ? JSON.parse(allPendingRequests) : [];

            return Promise.all(allPendingRequests
                .map(requestID => {
                    let requestOptions = Data.getStringVarible(requestID);
...
function clearOfflineDatabase() {
    return new Promise((resolve, reject) => {
        try {
            let allCachedRequests = Data.getStringVarible(TABLE_NAMES.CACHED_REQUESTS);
            let allPendingRequests = Data.getStringVarible(TABLE_NAMES.PENDING_REQUESTS);
