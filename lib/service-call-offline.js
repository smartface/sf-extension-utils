...
const requestCleaners = new WeakMap();
const offlineRequestHandlers = new WeakMap();
...
class OfflineRequestServiceCall extends ServiceCall {
    constructor(options) {
        super(options);
        if (options.offlineRequestHandler)
            this.offlineRequestHandler = options.offlineRequestHandler;
    }

    get offlineRequestHandler() {
        return offlineRequestHandlers.get(this);
    }

    set offlineRequestHandler(value) {
        return offlineRequestHandlers.set(this, value);
    }
...
class OfflineResponseServiceCall extends ServiceCall {
    ...
    get requestCleaner() {
        return requestCleaners.get(this);
    }

    set requestCleaner(value) {
        return requestCleaners.set(this, value);
    }
...
function clearOfflineDatabase() {
    return createAsyncTask(() => {
        let clearAllRequestsCommand = squel.delete()
            .from(TABLE_NAMES.REQUESTS)
            .toString();
        let clearAllResponsesCommand = squel.delete()
            .from(TABLE_NAMES.RESPONSES)
            .toString();
        let clearAllPendingRequestsCommand = squel.delete()
            .from(TABLE_NAMES.PENDING_REQUESTS)
            .toString();
        database.execute(clearAllRequestsCommand);
        database.execute(clearAllResponsesCommand);
        database.execute(clearAllPendingRequestsCommand);
    });
}
