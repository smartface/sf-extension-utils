...
function addPressEvent(target, event, options) {
    ...
    if (System.OS === "iOS") {
        ...
        //Fade effect for iOS
        if (options.ios.enableFadeEffect) {
            options.startTouchEffect = addFadePressEventForiOS.bind(target);
            options.endTouchEffect = clearFadePressEventForiOS.bind(target);
            target.__duration__ = options.ios.duration || 0.2;
            target.__maxOpacity__ = options.ios.maxOpacity || 0.3;
            target.__fadeEffectColor__ = options.ios.color;
        }
    }
...
function addFadePressEventForiOS() {
    const target = this;
    if (target.backgroundColor || target.__fadeEffectColor__) {
        if (!target.__pressEffectGeneratedColor__) {
        ...
        animation.toValue = target.__maxOpacity__;
        animation.duration = (target.__maxOpacity__ - currentOpacity) * target.__duration__ / target.__maxOpacity__;
        __SF_CATransaction.begin();
        __SF_CATransaction.setDisableActions(true);
        target.__pressEffectLayer__.opacity = target.__maxOpacity__;
        __SF_CATransaction.commit();
        target.__pressEffectLayer__.addAnimationForKey(animation, "opacity");
    }
}

function clearFadePressEventForiOS() {
    const target = this;
    if (target.__pressEffectLayer__) {
        if (!target.__pressEffectLayer__.getPresentationLayer()) {
        ...
        animation.fromValue = currentOpacity;
        animation.toValue = 0;
        animation.duration = currentOpacity * target.__duration__ / target.__maxOpacity__;
        __SF_CATransaction.begin();
        __SF_CATransaction.setDisableActions(true);
