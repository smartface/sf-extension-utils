 ...
 * @params {number} [options.duration] - Ripple effect requires duration before performing given event. This option specfic to Android.
 * @params {boolean} [options.useForeground] - iff this options is set to true, ripple effect added on top of the given target. No matter what given target contains, draw ripple effect over them. This option specfic to Android.
...
function addPressEvent(target, event, options) {
    ...
    options.useForeground = options.useForeground || false;
    options.rippleColor = options.rippleColor || Color.create("#d8d8d8");
    options.enableRippleEffect && applyRippleEffect.call(target, options.useForeground, options.rippleColor);
    ...
    target.onTouchEnded = (isInside) => {
        if (System.OS === "Android" && options.duration > 0) {
