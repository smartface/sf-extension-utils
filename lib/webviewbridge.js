 ...
 * @param {UI.WebView} [options.webView = new WebView()] - If not provided, it creates a new empty WebView instance. onChangedURL and onShow events of the WebView are set
 * @params {string} [options.scheme = "msg"] - URI scheme in wevView to communicate with webview
 * @params {string|IO.File} source - source to inject bridge code
 * @params {boolean} [parseResponses = false] - when false, the WebView.evaluateJS runs faster without parsing the executed JS code response
 * @params {boolean} [bounceEnabled = false] - when false bounce effect of the WebView is disabled
 * @example
...
const WebViewBridge = new extend(EventEmitter)(function(_super, options) {
    ...
    expect(options).to.have.property('source').that.satisfy((source) => {
    ...
    var ready = false;
    const source = options.source; //TODO: Enforce
